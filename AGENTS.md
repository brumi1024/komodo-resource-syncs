...
It is applied by a meta sync that `brumi1024/homelab-infra` creates and re-runs (the `komodo_gitops` role, part of `make bootstrap`), so a repository URL or branch changes here and nowhere else.
